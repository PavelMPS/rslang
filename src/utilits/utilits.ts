import { getWords, getUserWord, updateUserWord, createUserWord, getStatistics, updateStatistics, getAllUserWords } from '../api/api';
...
async function updateStatisticsByResults(userId: string | null, game: string, wordsInf: {newWords: number, learnedWords: number}) {
  ...
  if (game === sprint) {
    let maxLine: number = sprintGame.maxSeries;
    if (sprintGame.maxSeries < statisticInf.optional.sprint.maxLine) {
